...
# # Create a new job in the pipeline by updating the pipeline configuration
# curl -X PUT -u "username:$PAT" -H "Content-Type: application/json" -d "$NEW_JOB_DEFINITION" "$ORG_URL/$PROJECT_NAME/_apis/pipelines/$PIPELINE_ID/jobs/$NEW_JOB_NAME?api-version=6.0-preview.1"
